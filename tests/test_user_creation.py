...
class TestUserCreation:
    ...
    def test_create_existing_user(self, create_user):
        ...
        response = register_user(email=email, password=password, name=name)
        assert ResponseChecker.check_status_code(response, FORBIDDEN_CODE) and \
            ...
            f"Неожиданный ответ при попытке создать существующего пользователя. Код ответа: {response.status_code}, тело ответа: {response.json()}"
    ...
    def test_create_user_without_required_field(self, field):
        ...
        assert ResponseChecker.check_status_code(response, FORBIDDEN_CODE) and \
            ...
            f"Неожиданный ответ при попытке создать пользователя без {field}. Код ответа: {response.status_code}, тело ответа: {response.json()}"
